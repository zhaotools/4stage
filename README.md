...
基于周线结构、每日动态更新的四阶段趋势分类工具，覆盖主要A股ETF、沪深300成分股、加密资产，以及首批高流动性美股和美股ETF。
...
- [x] GitHub Pages部署工作流
- [x] 日线聚合周线，并每日更新本周动态K线
- [x] 30周均线、ATR、斜率、突破和波动特征
- [x] Stage 1～4评分模型
- [x] 有记忆的阶段状态机、过渡状态和两周确认
- [x] 42只主流ETF、8个主要指数和当前沪深300成分股
- [x] BTC、ETH、HYPE及MSTR、CRCL、HOOD、COIN
- [x] 22只高流动性美股和18只常用美股ETF
...
默认数据源无需令牌，会更新A股ETF、指数、当前沪深300成分股，以及配置中的加密货币、美股和美股ETF：

```bash
pnpm data:update
```

数据按日线下载、校验OHLC关系、聚合为周线，再运行阶段识别。本周尚未结束时，最新周K和阶段结果会随每日行情动态变化。也可以显式使用Tushare：
...
- 海外资产通过Yahoo行情适配器获取复权日线；所有市场均保留本周动态周线；
...
| Stage 4→2 | 强势修复站回长期均线上方，等待MA30转升确认 |
